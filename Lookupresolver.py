...
def build_lookup_map(parent_object: str, config: dict, output_dir: str = "output") -> dict:
    """
    Build a map from parent's id_column to External_ID__c.
    
    Returns:
        dict: {id_column_value: External_ID__c_value}
    """
    ...
    id_column = parent_config['id_column']
    
    if id_column not in parent_df.columns:
        print(f"  ERROR: id_column '{id_column}' not found in {parent_file}")
        return {}
    ...
    # Build map: id_column -> External_ID__c
    lookup_map = {}
    for _, row in parent_df.iterrows():
        id_val = row[id_column]
        ext_id = row['External_ID__c']
        if pd.notna(id_val) and pd.notna(ext_id):
            lookup_map[str(id_val).strip()] = str(ext_id).strip()
    
    print(f"  Built lookup map for '{parent_object}': {len(lookup_map)} records")
...
def resolve_lookups(object_name: str, config: dict, output_dir: str = "output"):
    ...
    for lookup_field, lookup_config in lookups.items():
        ...
        lookup_map = build_lookup_map(parent_object, config, output_dir)
